...
import { getPublicConfig, KNOWN_SLUGS } from "./configRegistry";
...
function ShellWithRouter({ config }: { config: AppShellConfig }) {
  ...
  // Bundled demo slugs stay on the offline mock; Nexus-booted apps get the
  // real client (real accounts, signups landing in the registration queue).
  const client = useMemo(
    () => (KNOWN_SLUGS.includes(config.slug) ? createMockClient(config.slug) : createNexusClient(config.slug)),
    [config.slug],
  );
