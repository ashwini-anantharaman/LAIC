...
export const SUIT_RESPONSES: TemplateItem[] = [
  auctionItem(
    ...
    "With three-card support raise 1♥/1♠ to two (6–10) or jump to three as a limit raise (10–12). With five trumps and a weak hand, jump straight to game preemptively. Each raise SHOWS its support length and strength, so partner can reason about the combined hands (the agreed trump suit, an 8-card fit, and the point total for slam decisions).",
    "agreement",
    // Split per major so each raise carries an explicit `shows` naming the
    // literal trump suit (a length promise on partner_last_bid_suit can't be
    // captured in meaning metadata, which is suit-literal) — this is what makes
    // fitEstablished / combinedHcp see the fit later in the auction.
    [
      rule(
        "limit-h",
        "Limit raise (hearts)",
        ctx("responder", { opening: is("1H"), partnerLast: is("1H") }),
        all(len("H", 3), tp(10, 12)),
        raise(3),
        30,
        { shows: { hcp: { min: 10, max: 12 }, suits: [{ suit: "H", min: 3 }] } },
      ),
      rule(
        "limit-s",
        "Limit raise (spades)",
        ctx("responder", { opening: is("1S"), partnerLast: is("1S") }),
        all(len("S", 3), tp(10, 12)),
        raise(3),
        30,
        { shows: { hcp: { min: 10, max: 12 }, suits: [{ suit: "S", min: 3 }] } },
      ),
      rule(
        "preempt-game-h",
        "Preemptive game raise (hearts)",
        ctx("responder", { opening: is("1H"), partnerLast: is("1H") }),
        all(len("H", 5), hcp(undefined, 9)),
        raise(4),
        31,
        { shows: { hcp: { max: 9 }, suits: [{ suit: "H", min: 5 }] } },
      ),
      rule(
        "preempt-game-s",
        "Preemptive game raise (spades)",
        ctx("responder", { opening: is("1S"), partnerLast: is("1S") }),
        all(len("S", 5), hcp(undefined, 9)),
        raise(4),
        31,
        { shows: { hcp: { max: 9 }, suits: [{ suit: "S", min: 5 }] } },
      ),
      rule(
        "single-h",
        "Single raise (hearts)",
        ...
        raise(2),
        32,
        { shows: { hcp: { min: 6, max: 9 }, suits: [{ suit: "H", min: 3 }] } },
      ),
      rule(
        "single-s",
        "Single raise (spades)",
        ...
        raise(2),
        32,
        { shows: { hcp: { min: 6, max: 9 }, suits: [{ suit: "S", min: 3 }] } },
      ),
    ],
  ),
  ...
  auctionItem(
    ...
    [
      ...
      rule(
        ...
        raise(2),
        33,
        { shows: { hcp: { min: 6, max: 9 }, suits: [{ suit: "D", min: 4 }] } },
      ),
      rule(
        "raise-c",
        ...
        raise(2),
        34,
        { shows: { hcp: { min: 6, max: 9 }, suits: [{ suit: "C", min: 5 }] } },
      ),
      rule(
        "one-nt",
        "1NT response",
