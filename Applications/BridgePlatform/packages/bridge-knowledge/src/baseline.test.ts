// Phase 3 acceptance: the golden-board harness runs the generated Beginner
...
async function generatedV0() {
  ...
  return (await store.getPackage(BEGINNER_NATURAL_PACKAGE_ID, "0.1.0"))!;
}
...
describe("Beginner Natural v0 golden-board baseline", () => {
  it("plays the golden boards with zero fallbacks (all decisions cited)", async () => {
    const record = await generatedV0();
