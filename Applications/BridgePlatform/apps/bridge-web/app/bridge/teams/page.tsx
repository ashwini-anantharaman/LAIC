...
export default async function TeamsPage({
  ...
  searchParams: Promise<{ invited?: string; who?: string; tab?: string; view?: string; provider?: string }>;
}) {
  const context = await getBridgeContext();
  if (!context) redirect("/welcome");
  if (await isFellowDemo()) redirect("/bridge/table");
  if (!canAccessAdminArea(context)) redirect("/bridge/home");
  const { invited, who, tab, view, provider } = await searchParams;

  const activeTab: "people" | "rg" | "catalog" = tab === "rg" ? "rg" : tab === "catalog" ? "catalog" : "people";
  const activeView: "grid" | "stack" = view === "stack" ? "stack" : "grid";
  // Catalog tab can edit either DOCUMENT: bridge's own, or the library
  // component's (platform-level, the single source of truth for library.*).
  const catalogProvider: "bridge" | "library" = provider === "library" ? "library" : "bridge";

  const programId = nexusProgramId(context);
  const live = nexusMode() === "http" && !!programId;

  // Management data (http mode only). The ROLE BUILDER uses the merged
  // inventory (bridge ∪ library); the catalog EDITOR edits one document.
  const [catalogue, editorDoc, roles, people] = live
    ? await Promise.all([
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      {!live ? (
        ...
        <div className="space-y-4">
          {/* Document switcher: bridge's own catalog vs the LIBRARY
              component's (shared across hosts; library.* single source). */}
          <div className="flex gap-2">
            {(
              ...
              <Link
                key={p.id}
                href={`/bridge/teams?tab=catalog${p.id === "library" ? "&provider=library" : ""}`}
                className={
                  p.id === catalogProvider
          ...
          {catalogProvider === "library" && (
            <p className="rounded border border-violet-200 bg-violet-50 px-3 py-2 text-xs text-violet-900">
              The library <b>component&apos;s own</b> catalogue — shared by every host platform.
              Saving requires platform-settings access on Nexus.
            </p>
          )}
          {editorDoc ? (
            <CatalogueEditor
            ...
            <p className="rounded-lg border border-[var(--line)] px-4 py-6 text-center text-sm text-neutral-500">Could not load the catalog.</p>
          )}
        </div>
      ) : (
        // ── Roles & Groups tab ─────────────────────────────────────────────
