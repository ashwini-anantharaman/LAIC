...
export default async function WelcomePage() {
  ...
  // An EMBEDDED session that died lands here — the reporter below tells the
  // host app so its /welcome watchdog re-launches, instead of the learner
  // staring at a sign-in page inside an iframe that can't sign in.
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col justify-center gap-8 p-8">
      {/* Unconditional: a dead session may have lost the embedded cookie
          too, and an unreported /welcome strands the host app's watchdog. */}
