// New Challenge — NATIVE, with the platform wizard's whole depth (M4; this
// grew from the one-screen M3f form).
//
...
export default function ChallengeNewScreen() {
  ...
  const draftOf = useCallback(
    (): ChallengeDraft => ({
      ...
      invites: [...invited].map((userId) => ({ userId, moderator: moderators.has(userId) })),
      editorBadge: false,
      ...(engine === "ben" ? { engine } : {}),
    }),
    [effectiveTitle, description, format, scoring, standings, boards, controls, invited, moderators, engine],
  );
  ...
  const create = useCallback(async () => {
    ...
    setError(null);
    try {
      await createChallenge(token, programId, draftOf(), draftEntryId ?? undefined);
          ...
          {view === "quick" && (
            <>
              <Text style={styles.quickNote}>
                Pick the boards and who&apos;s in — everything else takes its default: bid &
                play, IMPs, solver robots, spoiler-safe standings.
              </Text>
