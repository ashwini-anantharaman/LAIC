...
### 10.6 Coach Integration Requirements (what the Bridge Platform must provide the Coach)

The Coach is a separate, domain-agnostic service and does not implement any
bridge logic. For it to observe play and coach live/postmortem, the Bridge
Platform must expose the following. These extend §10.5 from "possible future
pattern" into the concrete contract the Coach depends on.

- **BR1 — Event subscription.** Let the Coach subscribe to the game event bus and receive `BridgeEventBase` events (§9.3) — `bid_action`, `play_action`, `session`, `deal_started`. The Coach maps these to a generic `ActivityEvent` (`eventId`, `bridgeSessionId`→`sessionId`, `actorId`, `eventType`, `timestamp`, `payload`→`action`, `domainId="bridge"`). Delivery is async; Bridge must not block on the Coach (§10.5).
- **BR2 — Evaluation facts.** Emit `BridgeActionEvaluation` (§14.2) per action — `judgment`, `confidence`, `relatedConceptIds`/`relatedSkillIds`, `matchedRuleIds`/`missedRuleIds`. **Bridge is the evaluator; the Coach consumes the verdict and never re-implements bridge rules** (§14.3). Provide a documented mapping from `judgment` (`aligned`/`reasonable_alternative`/`questionable`/`not_system_aligned`/…) to the Coach's correctness scale (`correct`/`acceptable`/`suboptimal`/`incorrect`) so the same intervention policy applies.
- **BR3 — Coach → Bridge response / render contract *(the currently-undefined seam)*.** Define how the Coach's output gets back into the table: a subscription callback (or response schema) carrying the intervention (`hint`/`nudge`/`question`) **or** a `tool_call`, correlated by `callId` and tagged with the coach instance/session. §10.5 and §18-Seq-9 today leave this as a deliverable — it must be specified for integration to be seamless (mirror of the Learning Platform's `/retrieve` contract).
- **BR4 — Tool catalogue + execution (host-executed).** Advertise bridge actions as tools (`name`, `description`, `inputSchema`, `policyHints`) — e.g. expand-hint, step-guided-replay, open-postmortem-section, load-similar-deal — and **execute** the Coach's proposed `ToolCall`s in the table UI, returning a `ToolResult` (`callId`-correlated). The Coach proposes and gates; Bridge runs its own code.
- **BR5 — Position / board context.** Provide the current `BridgePositionSnapshot` (auction, play, contract, declarer, dummy, trick state) as the activity context the Coach reasons over, and support PBN/LIN import so the Coach can drive guided replay.
- **BR6 — Progress signals → the Coach's learner model.** Emit `BridgeProgressSignal` (§13.6) to the Coach. **Constraint:** `bridgeSignalIds` must reference the Coach's own Event Log / Skill State, not a second separately-computed skill record, so the two platforms never diverge on what the learner mastered.
- **BR7 — Shared concept/skill taxonomy.** Use concept/skill ids that line up across evaluation, knowledge packages, and the learner model, so verdict → knowledge → mastery all key off the same ids.
- **BR8 — Timing hooks.** Provide before-commit and after-commit hooks so the Coach can intervene live (or defer to postmortem), honoring the non-blocking rule (§10.5) and real-time latency (the Coach's decision is instant; LLM phrasing is optional/deferrable).
- **BR9 — Identity + isolation.** Stamp `domainId="bridge"` and a Nexus learner identity on every event/signal so the Coach attributes and isolates bridge progress (a learner's bridge mastery never surfaces in another domain's view).

The Coach supplies, in return, everything under the Coaching Platform boundary (§2.2): coach runtime, learner context, intervention policy, hints/explanations, postmortems, recommendations, and traceability — none of which the Bridge Platform builds.
