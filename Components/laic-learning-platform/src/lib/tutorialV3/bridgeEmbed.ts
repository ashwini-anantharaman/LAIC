...
export function isBridgeEmbedPart(part: Pick<TutorialV3Part, 'type'>): boolean {
  return part.type === 'bridge-embed';
}

/** A fresh block for the Blocks row. */
export function makeBridgeEmbedPart(kind: BridgeEmbedKind, id: string): TutorialV3Part {
 ...
 */
export function configToPartFields(c: BridgeEmbedConfig): Partial<TutorialV3Part> {
  return {
