...
/**
 * A section's place in the lesson.
 *
 * Every section in a batch is generated by its own call, from the same source
 * units — so each one opens the topic from the beginning and the whole lesson
 * comes back as five sections all called "Introduction". A section needs to
 * know what has already been taught before it can teach the next thing.
 */
interface SectionArc {
  /** 1-based position among this lesson's sections. */
  index: number;
  total: number;
  /** Titles already written, in order, this run or an earlier one. */
  written: string[];
  /** Titles still to come, where the author named them. */
  upcoming: string[];
}

function arcPrompt(arc?: SectionArc): string | undefined {
  if (!arc || arc.total < 2) return undefined;
  const lines = [
    `This is section ${arc.index} of ${arc.total} in one lesson, and the sections are read in order.`,
  ];
  if (arc.written.length) {
    lines.push(
      `Already written, in order: ${arc.written.map((t) => `"${t}"`).join(', ')}.`,
      'Do not re-introduce the subject or repeat what those sections covered — continue from where they left off and teach the next thing.',
    );
  } else if (arc.index === 1) {
    lines.push('This is the opening section, so it may set the subject up.');
  }
  if (arc.upcoming.length) {
    lines.push(`Still to come: ${arc.upcoming.map((t) => `"${t}"`).join(', ')}. Leave those to their own sections.`);
  }
  lines.push('Give this section a heading that names what it teaches — never "Introduction" unless it genuinely is the opening.');
  return lines.join(' ');
}
...
async function generateSectionParts(opts: {
  ...
  highlights: any[];
  signal: AbortSignal;
  /** Where this section sits in the lesson, and what has already been written. */
  arc?: SectionArc;
}): Promise<TutorialV3Part[]> {
  const { draft, template, section, units, highlights, signal, arc } = opts;
  /*
 ...
 */
/**
 * Split the shared material between the sections of one batch.
 *
 * One markup pass produces one pool of units, and handing all of it to every
 * section asks five different calls to write the same lesson from the same
 * evidence — which is exactly what they do. A source document is already in a
 * teaching order, so each section takes a contiguous stretch of it, in order.
 *
 * The slices overlap by one unit at each seam: a section that begins mid-idea
 * reads better with the sentence that set it up, and the overlap is what keeps
 * the seams from landing mid-thought.
 */
function sliceUnitsForSections(units: ContentUnit[], count: number): ContentUnit[][] {
  if (count < 2 || units.length < count * 2) {
    // Too little material to divide without starving a section — share it all
    // and let the arc context keep the sections from repeating each other.
    return Array.from({ length: Math.max(1, count) }, () => units);
  }
  const per = Math.ceil(units.length / count);
  return Array.from({ length: count }, (_, i) => {
    const from = Math.max(0, i * per - (i > 0 ? 1 : 0));
    return units.slice(from, Math.min(units.length, (i + 1) * per));
  });
}
...
export async function runBatchGenerate(opts: {
  ...
  /*
    Every section of the lesson in reading order — not just the ones in this
    run. A section's place in the arc, and the stretch of source it teaches
    from, are facts about the tutorial; deriving them from the batch instead
    would mean retrying one failed section generated it as though it were the
    whole lesson, opening from the beginning all over again.
  */
  const allSections = outlineRows(draft)
    .filter((r): r is Extract<typeof r, { kind: 'section' }> => r.kind === 'section')
    .map((r) => r.section);
  const sectionIds = allSections.map((s) => s.id);
  const sectionSlices = markup.units?.length
    ? sliceUnitsForSections(markup.units, sectionIds.length)
    : null;

  /** Titles by section id — what has actually been written, updated as we go. */
  const writtenTitles = new Map<string, string>();
  for (const sec of allSections) {
    if ((sec.parts || []).length && !isPlaceholderSectionTitle(sec.title)) {
      writtenTitles.set(sec.id, sec.title);
    }
  }
  ...
  for (const target of targets) {
    ...
    try {
      ...
      if (target.kind === 'section') {
        const section = allSections.find((s) => s.id === target.id);
        if (!section) throw new Error('Section no longer exists.');
        const at = Math.max(0, sectionIds.indexOf(target.id));
        const sectionUnits = sectionSlices?.[at]?.length ? sectionSlices[at] : units;
        const arc: SectionArc = {
          index: at + 1,
          total: sectionIds.length,
          written: sectionIds.slice(0, at).map((id) => writtenTitles.get(id) || '').filter(Boolean),
          // Only names the author chose — a placeholder says nothing about what
          // is coming, and promising "Section 4" to the model is noise.
          upcoming: allSections
            .slice(at + 1)
            .map((x) => x.title || '')
            .filter((t) => t && !isPlaceholderSectionTitle(t)),
        };
        const prose = await generateSectionParts({
          draft, template, section, units: sectionUnits, highlights: markup.highlights, signal, arc,
        });
        const filled = await fillSectionEmbedSlots({
          section, parts: prose, units: sectionUnits, draft, signal,
        });
        ...
        */
        ...
        writtenTitles.set(target.id, generatedTitle || section.title);
