/** Tutorial V3 template editor — cloned from current fixed TutorialTemplateEditor. */
...
export function TutorialTemplateEditorV3({ initial, onSave, onCancel }: Props) {
  const seed = initial
    ? {
        name: initial.name,
  ...
  const handleSave = () => {
    ...
    const saved = saveCustomTutorialTemplate({
      ...
      sectionBlockRecipe,
      mediaSlots: deriveMediaSlots(cleanedRecipe),
      knobDefaults: {
        ...seed.knobDefaults,
                  ...
                  >
                    ...
                    {isOpen && (
                      <div className="px-3 pb-3 space-y-2.5" style={{ paddingLeft: 14, borderLeft: '3px solid #94A3B8' }}>
                        ...
                        {item.blockType === 'media' && (
                          <div>
                            <p style={{ fontSize: 11, fontWeight: 600, color: '#9AA3AF', marginBottom: 4 }}>Media type</p>
                            <div className="flex flex-wrap gap-1.5">
                              {([
                                { id: 'either' as const, label: 'Image or YouTube' },
                                { id: 'image' as const, label: 'Image only' },
                                { id: 'video' as const, label: 'YouTube only' },
                              ]).map((o) => (
                                <button
                                  key={o.id}
                                  type="button"
                                  onClick={() => patchAtomic(item.id, {
                                    media: {
                                      kind: o.id,
                                      hint: item.media?.hint || 'Optional media for this section',
                                    },
                                  })}
                                  className="px-2.5 py-1 rounded-full border"
                                  style={pill((item.media?.kind || 'either') === o.id)}
                                >
                                  {o.label}
                                </button>
                              ))}
                            </div>
                            <label style={{ fontSize: 11, fontWeight: 600, color: '#9AA3AF', display: 'block', marginTop: 8, marginBottom: 4 }}>
                              Slot hint (shown in Start from scratch)
                            </label>
                            <input
                              value={item.media?.hint || ''}
                              onChange={(e) => patchAtomic(item.id, {
                                media: {
                                  kind: item.media?.kind || 'either',
                                  hint: e.target.value,
                                },
                              })}
                              placeholder="e.g. Diagram of a trick or short demo clip"
                              className="w-full rounded-xl px-3 py-2"
                              style={field}
                            />
                          </div>
