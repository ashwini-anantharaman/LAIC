...
import { getTutorialTemplate, templateUsesCompositeRecipe, toFlatSectionBlockRecipe } from '../../../../lib/tutorialV2/tutorialTemplates';
import {
  bumpAuthorMode,
...
import type { TutorialV2Draft, TutorialV2Part, V2Section, V2TopLevelSlot } from '../../../../lib/tutorialV2/types';
...
export function TutorialV2SectionWorkspace({
  draft,
  section,
  onChangeSection,
  onBack,
  ...
  section: V2Section;
  onChangeSection: (patch: Partial<V2Section>) => void;
  onBack: () => void;
  ...
  slot?: V2TopLevelSlot;
  onChangeSlot?: (patch: Partial<V2TopLevelSlot>) => void;
  ...
  const runRefine = async (part: TutorialV2Part) => {
    setError(null);
    setBusy(true);
    try {
  ...
  const runGenerateSection = async () => {
    ...
    try {
      ...
      onChangeSection({
        parts: collected as TutorialV2Part[],
        units,
      ...
      {tab === 'write' && (
        <WritePane
          ...
          onAddPart={(kind) => {
            ...
            const part: TutorialV2Part = kind === 'text'
...
function WritePane({
  ...
  onEnsure: () => void;
  onChangePart: (id: string, patch: Partial<TutorialV2Part>) => void;
  ...
  onRefine: (p: TutorialV2Part) => void;
  busy: boolean;
...
function MediaSlotEditor({
  ...
  part: TutorialV2Part;
  onChange: (patch: Partial<TutorialV2Part>) => void;
}) {
...
function GeneratePane({
  ...
  pool: TutorialV2Draft['sourcePool'];
  picked: string[];
