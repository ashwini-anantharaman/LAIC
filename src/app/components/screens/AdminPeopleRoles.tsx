...
export function AdminPeopleRoles() {
  const [catalogue] = useState(() => loadCatalogue());
  ...
  const deleteCustom = (id: string) => {
    setPolicy(deleteCustomPolicyRole(id));
  ...
  const saveRole = (role: {
    ...
    const next = upsertCustomPolicyRole({
      id: role.id,
      name: role.name,
    ...
    setPolicy(next);
    const demo = listPolicyDemoAccounts({ customOnly: true }).find((a) =>
      next.roles.some((r) => r.id === a.policyRoleId && r.name === role.name.trim()),
    ) || listPolicyDemoAccounts({ customOnly: true }).slice(-1)[0];
    fireToast(
      demo
        ? `Role saved · login ${demo.email} / ${demo.password}`
        : 'Role saved to access policy',
    );
  };
